...
使い方:
    ...
    python -m tests.run_all_tests --stage 1        # Stage 1 のみ
...
def extract_counts(stdout: str) -> Tuple[Optional[int], Optional[int]]:
    """
    ...
    """
    # カスタムランナー形式を優先
    m = re.search(r'pass\s*=\s*(\d+)\s+fail\s*=\s*(\d+)', stdout)
    if m:
        return int(m.group(1)), int(m.group(2))

    # unittest 形式
    m_ran = re.search(r'Ran\s+(\d+)\s+tests?', stdout)
    if m_ran:
...
def run_one_test(
    ...
    try:
        ...
        pass_count, fail_count = extract_counts(proc.stdout)
        return TestResult(
            test=test,
