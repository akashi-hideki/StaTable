"""割り込み処理・デバイスリソース・タイマ設定の管理ダイアログ（状態遷移編集と同構造）"""
...
class InterruptEditDialog(QDialog):
    def __init__(self, parent=None, event_names=None, interrupt: Optional[InterruptHandlerDef] = None):
        ...
        layout.addWidget(QLabel("動作コード（割り込み内で実行）:"))
        self.actions_edit = QTextEdit()
        self.actions_edit.setAcceptRichText(False)
        self.actions_edit.setFont(QFont("Consolas", 10))
        if interrupt:
            self.actions_edit.setPlainText(interrupt.actions)
...
class InterruptHandlerEditDialog(QDialog):
    ...
    def refresh_interrupt_table(self):
        ...
        for intr in self.interrupts:
            ...
            action_display = intr.actions.replace('\n', ' ; ')
            self.interrupt_table.setItem(row, 4, QTableWidgetItem(action_display))
    ...
    def on_interrupt_double_clicked(self, row, col):
        ...
        dlg = InterruptEditDialog(self, event_names=self.event_names, interrupt=target)
    ...
    def add_interrupt(self):
        dlg = InterruptEditDialog(self, event_names=self.event_names)
        if dlg.exec() == QDialog.Accepted:
