...
"""
動作編集メインダイアログ（共有ライブラリ対応・パレット編集対応・デバッグ強化版）
"""
...
class ActionEditorDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        logger.debug(f"condition_names = {condition_names}")
        ...
        # シグナル接続
        logger.debug("Connecting canvas signals...")
        ...
        # パレットからの編集要求
        logger.debug("Connecting palette signals...")
        ...
        logger.debug("Palette signals connected")
    ...
    def canvas_auto_align(self):
        """キャンバスのノードを自動整列"""
        logger.debug("canvas_auto_align called")
    ...
    def _on_edit_function_requested(self, name: str):
        ...
        if rf is None:
            ...
            return
        logger.debug(f"  Found role function: {rf.name}")
        ...
        logger.debug(f"  global_vars={global_vars}, events={events}, literals={literals}")
        ...
        logger.debug("  Opening RoleFunctionEditDialog...")
        if dlg.exec() == QDialog.Accepted:
            # ライブラリは既に更新されているので、パレットの表示を更新
            logger.debug("  RoleFunctionEditDialog accepted")
            function_names = [rf.name for rf in self.role_function_library.list_all()]
            self.palette.refresh_lists(role_functions=function_names)
        else:
            logger.debug("  RoleFunctionEditDialog cancelled")
    ...
    def _on_edit_transition_requested(self, name: str):
        ...
        if ct is None:
            ...
            return
        logger.debug(f"  Found condition template: {ct.name}, condition='{ct.condition}'")
        ...
        logger.debug("  Opening ConditionBuilderDialog...")
        if dlg.exec() == QDialog.Accepted:
            new_condition = dlg.get_condition_text()
            new_name = dlg.get_event_name()
            # ライブラリのテンプレートを更新
            logger.debug(f"  ConditionBuilderDialog accepted: new_condition='{new_condition}', new_name='{new_name}'")
            ct.condition = new_condition
            # 名前が変わった場合はライブラリを更新
            if new_name and new_name != ct.name:
                logger.debug(f"  Renaming condition template: {ct.name} -> {new_name}")
                self.condition_library.remove(ct.name)
                ct.name = new_name
                self.condition_library.add(ct)
            # パレット更新
            condition_names = [c.name for c in self.condition_library.list_all()]
            self.palette.refresh_lists(condition_templates=condition_names)
        else:
            logger.debug("  ConditionBuilderDialog cancelled")
    ...
    def _on_node_edit_requested(self, node: FlowNodeItem):
        ...
        if node.item_type == "function":
            ...
            if flow_item:
                ...
                logger.debug(f"  function flow_item name={flow_item.name}")
                rf = self.role_function_library.get(flow_item.name)
                if rf is None:
                    logger.debug(f"  Role function not in library, creating temporary: {flow_item.name}")
                ...
                if dlg.exec() == QDialog.Accepted:
                    ...
                    logger.debug(f"  Role function updated: {updated_rf.name}")
    ...
    def _on_node_delete_requested(self, node: FlowNodeItem):
        logger.debug("Node delete requested")
    ...
    def _on_node_duplicate_requested(self, node: FlowNodeItem):
        """ノードを複製して下に追加"""
        logger.debug("Node duplicate requested")
        flow_item = node.flow_item
        if flow_item:
            # 深いコピーを作成
            import copy
    ...
    def _on_node_move_down_requested(self, node: FlowNodeItem):
        """ノードを下へ移動"""
        logger.debug("Node move down requested")
        flow_item = node.flow_item
        if flow_item and flow_item in self.draft.flow_items:
            idx = self.draft.flow_items.index(flow_item)
