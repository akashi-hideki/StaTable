...
新設計への変更後、インポートエラーや基本的な不整合を検出する。
"""
...
def test_import_global_defs():
    ...
    # InterruptHandlerDef は event_names を持つ
...
def test_import_xml_io():
    ...
    assert sm2.transitions[0].condition == ""

    # GlobalDefinitions の往復
    defs = GlobalDefinitions()
    defs.variables.append(SystemVariable(name="temp", type="uint16_t"))
...
def test_transition_list_dialog_creation(app):
    ...
    trans = Transition(source="Idle", event="START", condition="x > 0", target="Active")
