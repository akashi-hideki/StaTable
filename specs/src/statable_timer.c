...
typedef struct {
    ...
    uint16_t g_tick_1s; /* 1s */
    uint16_t g_hs_1ms; /* 1ms */
} TimerVariables_t;
...
void Timer_Update(SystemContext_t *ctx)
{
    ...
    ctx->data.g_tick_1s = (uint16_t)(ctx->data.g_system_tick / 1000);
    ctx->data.g_hs_1ms = (uint16_t)(ctx->data.g_high_speed_tick / 10);
}